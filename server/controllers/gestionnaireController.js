...
exports.articlepost = async (req, res) => {
  ...
  db.query(
    "INSERT INTO article (date_art,titre_art,description_art,image_art,auteur_art)VALUES(?,?,?,?,?)",
    [
      req.body.date,
      req.body.titre,
...
exports.but = async (req, res) => {
  ...
  try {
    ...
    const new_score = await dbQuery(
      "SELECT score_eq1,score_eq2 FROM `match` WHERE id_ma=? ",
      [matchId]
    );
    const clubs = await dbQuery(
      "SELECT equipe_1,equipe_2 FROM `match` WHERE id_ma=? ",
      [matchId]
    );
    ...
    //  console.log(equipes);
    const equipe_encaisse =
      req.body.equipe === equipes.equipe1 ? equipes.equipe2 : equipes.equipe1;
    // console.log(equipe_encaisse);

    const but_p = await dbQuery(
      "UPDATE equipe SET nbr_but_p_eq= nbr_but_p_eq+1 WHERE nom_eq=?",
      [req.body.equipe]
    );
    const but_c = await dbQuery(
      "UPDATE equipe SET nbr_but_c_eq= nbr_but_c_eq+1 WHERE nom_eq=?",
      [equipe_encaisse]
    );
    ...
    io.emit("score", {
      score1: new_score[0].score_eq1,
      score2: new_score[0].score_eq2,
    });
  } catch (error) {
    console.error("Error in match route:", error);
    res.status(500).send("Internal Server Error");
...
exports.rouge = async (req, res) => {
  ...
  try {
    ...
    res.redirect(`/match/${gestId}/${matchId}`);
    io.emit("carton_rouge", {
      joueur: nom_joueur_rouge,
      equipe: req.body.equiperouge,
    });
  } catch (error) {
    console.error("Error in match route:", error);
    res.status(500).send("Internal Server Error");
...
exports.jaune = async (req, res) => {
  ...
  try {
    ...
    res.redirect(`/match/${gestId}/${matchId}`);
    io.emit("carton_jaune", {
      joueur: nom_joueur_jaune,
      equipe: req.body.equipejaune,
    });
  } catch (error) {
    console.error("Error in match route:", error);
    res.status(500).send("Internal Server Error");
...
exports.fin = async (req, res) => {
  ...
  db.query(
    "SELECT equipe_1 , equipe_2, score_eq1, score_eq2 FROM `match` WHERE id_ma=?",
    [matchId],
    (err, equipes) => {
      if (err) {
      ...
      if (equipes[0].score_eq2 >= equipes[0].score_eq1) {
        db.query(
          "UPDATE equipe SET m_gagner_eq=m_gagner_eq+1 WHERE nom_eq=?",
          [equipes[0].equipe_2],
          (err, result) => {
            if (err) {
            ...
            db.query(
              "UPDATE equipe SET m_perdu_eq=m_perdu_eq+1 WHERE nom_eq=?",
              [equipes[0].equipe_1],
              (err, perdu) => {
                if (err) {
...
exports.changement = async (req, res) => {
  ...
  io.emit("changement", {
    equipe: req.body.equipeChange,
    joueurIN: req.body.joueurIN,
    joueurOUT: req.body.joueurOUT,
  });
